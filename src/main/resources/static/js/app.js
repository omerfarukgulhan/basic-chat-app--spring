...
function connect(event) {
    ...
    fullName = document.querySelector('#full-name-input').value.trim();
...
function appendUserElement(user, connectedUsersList) {
    ...
    unreadMessages.classList.add('unread-messages', 'hidden');

    listItem.appendChild(userImage);
    listItem.appendChild(usernameSpan);
    listItem.appendChild(unreadMessages);

    listItem.addEventListener('click', onUserItemClick);
...
function onUserItemClick(event) {
    ...
    const unreadMessages = clickedUser.querySelector('.unread-messages');
...
async function fetchAndDisplayUserChat() {
    const response = await fetch(`/messages/${username}/${selectedUserId}`);
...
async function onMessageReceived(payload) {
    ...
    if (notifiedUser && !notifiedUser.classList.contains('active')) {
        const unreadMessages = notifiedUser.querySelector('.unread-messages');
